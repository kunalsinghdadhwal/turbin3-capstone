...
describe("multisig-vault", () => {
  ...
  async function expectError(fn: () => Promise<any>, errorCode: string) {
    try {
      ...
      if (err.message?.includes("Expected transaction to fail")) throw err;
    ...
    it("fails with duplicate signers", async () => {
      ...
      await expectError(
        () =>
          program.methods
            .initialize(
              [signer2.publicKey, signer2.publicKey, signer3.publicKey],
              2
            )
            .accounts({
              creator: c.publicKey,
  ...
  describe("create_proposal", () => {
    ...
    it("creates a proposal with a price condition (proposal 2)", async () => {
      ...
      const prop = await program.account.proposal.fetch(pricedProposalPda);
      expect(prop.priceCondition).to.not.be.null;
      expect(prop.priceCondition.feedId).to.deep.equal(SOL_USD_FEED_ID);
      expect(prop.priceCondition.minPrice.toNumber()).to.equal(15_000_000_000);
  ...
  describe("reject_proposal", () => {
    ...
    it("signer rejects a proposal", async () => {
      ...
      const prop = await program.account.proposal.fetch(rejectProposalPda);
  ...
  describe("execute_sol_proposal", () => {
    ...
    it("executes an approved proposal, SOL transferred to recipient", async () => {
      const vaultBefore = await connection.getBalance(vaultPda, "confirmed");
      ...
      const vaultAfter = await connection.getBalance(vaultPda, "confirmed");
  ...
  describe("execute_token_proposal", () => {
    ...
    it("executes approved SPL token transfer", async () => {
      const vaultAta = getAssociatedTokenAddressSync(testMint, vaultPda, true);
  ...
  describe("cancel_proposal", () => {
    ...
    it("proposer cancels an Active proposal", async () => {
      ...
      const prop = await program.account.proposal.fetch(cancelProposalPda);
